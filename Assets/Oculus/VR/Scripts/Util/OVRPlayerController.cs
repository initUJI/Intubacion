...
public class OVRPlayerController : MonoBehaviour
{
	...
	private bool _stepSound = false;

	[SerializeField] UnityEvent f_stepSoundEvent;
	...
	IEnumerator Steps()
	{
		while (true)
		{
			...
			if (_stepSound) f_stepSoundEvent?.Invoke();
			yield return null;
		}
	}
	void Awake()
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
			...
			{ MoveScale = 0.70710678f; _stepSound = true; }

			// No positional movement if we are in the air
			if (!Controller.isGrounded)
			{ MoveScale = 0f; _stepSound = false; }
			...
			if (primaryAxis.magnitude > 0.1f) _stepSound = true;
			else _stepSound = false;
